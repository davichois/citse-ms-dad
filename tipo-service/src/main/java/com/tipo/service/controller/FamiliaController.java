...
public class FamiliaController {
    ...
    public ResponseEntity<List<Familia>> findAll(){
        return ResponseEntity.ok().body(familiaServiceImp.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<Familia> findById(@PathVariable int id){
        Familia familia = familiaServiceImp.findById(id);

        if (familia != null){
            return ResponseEntity.ok().body(familiaServiceImp.findById(id));
        }else{
